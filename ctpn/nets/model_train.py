...
def load_model():
    gctpn = tf.Graph()
    with gctpn.as_default():
        input_image = tf.placeholder(tf.float32, shape=[None, None, None, 3], name='input_image')
        input_im_info = tf.placeholder(tf.float32, shape=[None, 3], name='input_im_info')

        global_step = tf.get_variable('global_step', [], initializer=tf.constant_initializer(0), trainable=False)

        bbox_pred, cls_pred, cls_prob = model(input_image)

        variable_averages = tf.train.ExponentialMovingAverage(0.997, global_step)
        saver = tf.train.Saver(variable_averages.variables_to_restore())

        sess = tf.Session(config=tf.ConfigProto())
        ckpt_state = tf.train.get_checkpoint_state('ctpn/checkpoints_mlt/')
        model_path = os.path.join('ctpn/checkpoints_mlt/', os.path.basename(ckpt_state.model_checkpoint_path))
        saver.restore(sess, model_path)
    return sess, input_image, input_im_info, bbox_pred, cls_prob

sess, input_image, input_im_info, bbox_pred, cls_prob = load_model()
